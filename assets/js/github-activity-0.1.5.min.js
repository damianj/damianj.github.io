...
function cmn(a, b, c, d, e, f) {
    return b = add32(add32(b, a), add32(d, f)),
    add32(b << e | b >>> 32 - e, c)
}
...
var GitHubActivity = function() {
    ...
    var a = {}
      , b = {
        renderLink: function(a, b, c) {
            ...
            Mustache.render('<a class="' + c + '" href="{{url}}" target="_blank">{{{title}}}</a>', {
                url: a,
                title: b
            })
        },
        ...
        getMessageFor: function(a) {
            var c = a.payload;
            if (a.repoLink = b.renderGitHubLink(a.repo.name),
            a.userGravatar = Mustache.render('<div class="gha-gravatar-user"><img src="{{url}}" class="gha-gravatar-small"></div>', {
                url: a.actor.avatar_url
            }),
            c.ref && (a.branch = "refs/heads/" === c.ref.substring(0, 11) ? c.ref.substring(11) : c.ref,
            a.branchLink = b.renderGitHubLink(a.repo.name + "/tree/" + a.branch, a.branch) + " at "),
            c.commits) {
                ...
                c.commits.forEach(function(d, e) {
                    return d.message.length > 66 && (d.message = d.message.substring(0, 66) + "..."),
                    2 > e ? (d.shaLink = b.renderGitHubLink(a.repo.name + "/commit/" + d.sha, d.sha.substring(0, 6), "gha-sha"),
                    void (d.committerGravatar = Mustache.render('<img class="gha-gravatar-commit" src="" width="16" />'))) : (c.commits.splice(2, c.size),
                    !1)
                })
            }
            if (c.issue) {
            ...
            if (c.pull_request) {
                ...
                if (a.pullRequestLink = b.renderLink(g.html_url, a.repo.name + "#" + g.number),
                a.mergeMessage = "",
            ...
            if (c.comment && c.comment.body && (a.comment = c.comment.body,
            a.comment.length > 150 && (a.comment = a.comment.substring(0, 150) + "..."),
            ...
            if ("ReleaseEvent" === a.type && (a.tagLink = b.renderLink(c.release.html_url, c.release.tag_name),
            a.zipLink = b.renderLink(c.release.zipball_url, "Download Source Code (zip)")),
            "GollumEvent" === a.type) {
                var i = c.pages[0];
                a.actionType = i.action,
                a.message = a.actionType.charAt(0).toUpperCase() + a.actionType.slice(1) + " ",
                a.message += b.renderGitHubLink(i.html_url, i.title)
            }
            "FollowEvent" === a.type && (a.targetLink = b.renderGitHubLink(c.target.login)),
            "ForkEvent" === a.type && (a.forkLink = b.renderGitHubLink(c.forkee.full_name)),
            "MemberEvent" === a.type && (a.memberLink = b.renderGitHubLink(c.member.login)),
            c.gist && (a.actionType = "fork" === c.action ? c.action + "ed" : c.action + "d",
            a.gistLink = b.renderLink(c.gist.html_url, "gist: " + c.gist.id));
        ...
        getHeaderHTML: function(a) {
            return a.name ? a.userNameLink = b.renderLink(a.html_url, a.name) : a.withoutName = " without-name",
            a.userLink = b.renderLink(a.html_url, a.login),
            a.gravatarLink = b.renderLink(a.html_url, '<img src="' + a.avatar_url + '">'),
            ...
            c.onreadystatechange = function() {
                if (4 === c.readyState)
                    if (c.status >= 200 && c.status < 300) {
                        ...
                        b("request for " + a + " yielded status " + c.status)
            }
            ,
            c.onerror = function() {
    ...
    return a.feed = function(a) {
        ...
        if (a.repository && (g = "https://api.github.com/repos/" + a.username + "/" + a.repository + "/events"),
        a.clientId && a.clientSecret) {
            var h = "?client_id=" + a.clientId + "&client_secret=" + a.clientSecret;
            f += h,
            g += h
        }
        if (a.eventsUrl && (g = a.eventsUrl),
        "object" == typeof a.templates)
            for (var i in templates)
                "string" == typeof a.templates[i] && (templates[i] = a.templates[i]);
