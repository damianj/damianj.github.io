...
let GitHubActivity = (function() {
  ...
  let methods = {
    ...
    getMessageFor: function(data) {
      ...
      if (p.commits) {
        let shaDiff = p.before + '...' + p.head;
        let length = p.commits.length;
        if (length === 2) {
      ...
      if (p.issue) {
        let title = data.repo.name + "#" + p.issue.number;
      ...
      if (p.pull_request) {
        ...
        if (p.pull_request.merged) {
          p.action = "merged";
          let message = '{{c}} ' + pluralize('commit', pr.commits) + ' with {{a}} ' + pluralize('addition', pr.additions) + ' and {{d}} ' + pluralize('deletion', pr.deletions);
      ...
      let message = Mustache.render(templates[data.type], data);
      let timeString = millisecondsToStr(new Date() - new Date(data.created_at));
      let icon;
      ...
      let activity = { message: message, icon: icon, timeString: timeString, userLink: methods.renderGitHubLink(data.actor.login) };
    ...
    writeOutput: function(selector, content) {
      let div = selector.charAt(0) === '#' ? document.getElementById(selector.substring(1)) : document.getElementsByClassName(selector.substring(1));
  ...
  obj.feed = function(options) {
    ...
    if (options.clientId && options.clientSecret) {
      let authString = '?client_id=' + options.clientId + '&client_secret=' + options.clientSecret;
      userUrl   += authString;
    ...
    methods.getOutputFromRequest(eventsUrl, function(error, output) {
      if (error) {
        ...
        let limit = options.limit != 'undefined' ? parseInt(options.limit, 10) : null;
